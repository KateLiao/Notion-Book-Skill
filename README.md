...
| 把 Apple Books 高亮 / 笔记同步到 Notion | 写入书籍页面的 `Apple Books 高亮与笔记` 模块，重复同步时自动去重 |

## 你会得到什么

- 一个 `Reading` / `阅读｜Reading` 风格的 Notion 阅读页面
- 标准 `书籍总览` 数据库，含 `➡️在读`、`🌟完成阅读`、`📎To Read List` 三个视图
- 新书页面自动带有 `Apple Books 高亮与笔记`、`批注与草稿`、`读书笔记` 三个可折叠三级标题
- `读书笔记` 下预置四个四级标题，帮助你边读边整理结构化笔记
- 可复用的 Codex / Hermes / OpenClaw Skill 指令与脚本
- 默认安全策略：只补空字段或修复明确损坏的封面，不覆盖评分、进度、日期、笔记等已有内容
- 封面优先取作者官网或出版社原图，并校验文件签名；不再把 Open Library、Google Books 或搜索引擎代理图作为自动来源
...
Apple Books 重复导入时只会处理 `Apple Books 高亮与笔记` 模块：已有摘录会跳过，新摘录会追加；`批注与草稿` 和 `读书笔记` 中你手写的内容不会被修改。
...
书名与目录名不同、或已经找到作者/出版社封面时，可以把核实结果交给同一条命令。脚本仍会在写入前验证封面：

```bash
node scripts/notion_book_completer.mjs add-books \
  --status "To read list" \
  --lookup-title "Catalog title" \
  --author "作者" \
  --cover-url "https://publisher.example/cover.jpg" \
...
## 书籍页面模板与 Apple Books 同步

新建书籍页面时，Skill 会自动创建一套读书笔记骨架：

- `Apple Books 高亮与笔记`：可折叠三级标题，用于存放 Apple Books 标注
- `批注与草稿`：可折叠三级标题，用于阅读过程中临时记录想法
- `读书笔记`：可折叠三级标题，内含四个四级标题：
  - `一、这本书整体在谈什么？`
  - `二、作者具体是怎样展开他的观点的？`
  - `三、作者说得对吗？`
  - `这和我有什么关系？`

Apple Books 导入会按样板生成内容：原文使用引用块，个人笔记单独放在下一段，`我的笔记：` 加粗，日期以灰色显示。

重复导入时，脚本只检查并更新 `Apple Books 高亮与笔记` 这个模块：

- 已存在的摘录会自动跳过
- 新摘录会追加到该模块末尾
- `批注与草稿`、`读书笔记` 中的手写内容不会被修改
- 需要重建 Apple Books 模块时，可显式使用 `--force`，它也只会刷新该模块的子块
...
├── references/cover-sources.md            # 封面来源优先级、禁用来源与校验规则
